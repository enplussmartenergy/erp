...
import { buildAirCompPdf } from "./buildAirCompPdf";
...
export default function PdfPreviewJS({ onClose, fileName = "report.pdf", building, reportMeta, report }) {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        setError("");
        const blob = await buildAirCompPdf(deps);
        if (!alive) return;
