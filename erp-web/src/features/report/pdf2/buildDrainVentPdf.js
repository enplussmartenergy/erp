// src/features/report/pdf2/buildDrainVentPdf.js
...
/**
 * ❌ 기존 firstPhotoUrl: id가 비어있으면 "아무 사진"을 fallback으로 가져와버림
 * ✅ 오배수는 현황사진을 "criteria_photo만" 쓰도록 strict로 분리
 */
...
/* ✅ autoTable 옵션 병합: merged 먼저, opt가 최종 승리 */
function headBodyTable(doc, opt, styleOverride) {
  ...
  autoTable(doc, {
    ...merged,
    ...opt,
    margin: { ...merged.margin, ...(opt?.margin || {}), left: SAFE.L, right: SAFE.R },
...
function photoGrid(doc, { title = "육안 점검", top = 32, rows = 2, cols = 2, gap = 5, captionH = 6, items = [], images = [], resultLines = [] }) {
  ...
  const lines = Math.max(1, resultLines && resultLines.length ? resultLines.length : 1);
...
function renderCover(doc, { pageNo, totalPages, dateTxt, placeLabel, engineer }) {
  pageChrome(doc, {
    title: "오·배수 동기 및 우수 배출 설비 성능 점검표",
    page: pageNo,
    total: totalPages,
  });
  ...
  headBodyTable(
    doc,
    {
      startY: (doc.lastAutoTable?.finalY || FRAME.T + 24) + 4,
      head: [["구 분", "점검내용", "점검결과"]],
      body: rows,
  ...
  headBodyTable(
    doc,
    {
      startY: (doc.lastAutoTable?.finalY || FRAME.T + 120) + 2,
      head: [["추진사항 목록", "내용"]],
      body: [
        ["1.", "오·배수 동기 및 우수 배출 설비 성능 점검 단계 및 기준"],
        ["2.", "육안 점검표"],
        ["3.", "배수펌프 작동 점검표"],
      ],
  ...
  const guide = [
    "1. 유지관리지침서와 기계설비 유지관리 및 성능점검 대상 현황표의 적합여부를 참고하여 점검결과를 작성한다.",
    "2. 점검결과에는 [적합 ○, 부적합 ×, 해당없음 /]을 표기한다.",
    "3. 배수펌프 점검은 점검 시점 및 측정 데이터까지를 포함하여 첨부한다.",
  ].join("\n");

  headBodyTable(
    doc,
    {
      startY: (doc.lastAutoTable?.finalY || FRAME.T + 150) + 2,
      head: [["작성 방법", "내용"]],
      body: [["", guide]],
      columnStyles: { 0: { cellWidth: 26 } },
    },
  ...
  const startY = (doc.lastAutoTable?.finalY || FRAME.T + 175) + 2;
...
function renderCriteria(doc, { pageNo, totalPages, criteriaPhoto }) {
  pageChrome(doc, {
    title: "1. 오·배수 동기 및 우수배수설비 성능 점검 단계 및 점검 기준",
    page: pageNo,
    total: totalPages,
  });
  ...
  const H = doc.internal.pageSize.getHeight();
  const baseY = (doc.lastAutoTable?.finalY || FRAME.T + 80) + 6;
  ...
  try {
    ...
    else throw new Error("no criteria image");
  } catch {
...
/* ==================== ✅ 마스터용 렌더러 ==================== */
export async function renderDrainVent(doc, { building, reportMeta, report, schema = [], __page } = {}) {
  if (!doc) throw new Error("renderDrainVent: doc is required");
  ...
  renderCover(doc, {
    ...
    totalPages,
    dateTxt,
    placeLabel: report?.meta?.label || building?.name || "",
  ...
  // ✅ 여기 핵심: criteria_photo만 사용 (다른 슬롯 fallback 금지)
  ...
  for (let i = 0; i < (schema?.length || 0); i++) {
    ...
    pageChrome(doc, { title: sec.title || "육안 점검", page: pageNo, total: totalPages });

    const urls = await toUrls(sec.slots || [], photoSlots);

    const resultLines = (notes?.[sec.id] || notes?.[sec.noteKey] || "")
      .split(/\r?\n/)
      .map((s) => s.trim())
      .filter(Boolean);

    photoGrid(doc, {
      title: sec.gridTitle || "육안 점검",
      rows: sec.rows ?? 2,
      cols: sec.cols ?? 2,
      items: sec.slots || [],
      images: urls,
...
/* ==================== ✅ 단독 빌더 ==================== */
export async function buildDrainVentPdf({ building, reportMeta, report, schema = [] } = {}) {
  const doc = new jsPDF({ unit: "mm", format: "a4" });

  const totalPages = 2 + (schema?.length || 0); // 표지1 + 기준1 + 섹션들
  const pageNoStart = 1;

  await ensureFonts(doc);
  setKR(doc);

  await renderDrainVent(doc, {
    building,
    reportMeta,
    report,
    schema,
    __page: { pageNoStart, totalPages },
  });
